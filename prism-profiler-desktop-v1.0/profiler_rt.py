...
Module name : Real Time predictions
Author: Yanis Zirem
...
def convert_raw_to_mzml_rt(raw_dir, output_dir):
    # Ensure session state variable is initialized as a set
    if 'processed_files' not in st.session_state or not isinstance(st.session_state['processed_files'], set):
        st.session_state['processed_files'] = set()

    # List all .raw files not yet processed
    raw_files = [
        f for f in os.listdir(raw_dir)
        if f.lower().endswith('.raw') and f not in st.session_state['processed_files']
    ]

    if not raw_files:
        st.info("No new .raw files found to convert.")
        return False
    ...
    for raw_file in raw_files:
        ...
        try:
            ...
            st.success(f"✅ Converted {raw_file} to mzML.")
        except subprocess.CalledProcessError as e:
            st.error(f"❌ Conversion failed for {raw_file}: {e.stderr or e}")
        except Exception as e:
            st.error(f"⚠️ Unexpected error for {raw_file}: {e}")
