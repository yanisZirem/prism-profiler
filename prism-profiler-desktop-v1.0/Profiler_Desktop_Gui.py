        ...
        with st.expander("**🌋 Volcano Plot**", expanded=st.session_state.get("show_volcano", False)):
            ...
            if "volcano_data" in st.session_state:
                try:
                    ...
                    # Display dataframe of significant features
                    if significant_features and "data" in st.session_state:
                        significant_features = list(significant_features)
                        class_column = st.session_state.get("class_column", "Class")

                        missing_cols = [f for f in significant_features if f not in st.session_state["data"].columns]
                        if missing_cols:
                        ...
                        if class_column in st.session_state["data"].columns:
                            significant_data = st.session_state["data"][[class_column] + significant_features]
                            st.write("**DataFrame with Significant Features:**")
                            st.dataframe(significant_data, hide_index=True)
                        else:
                            st.error("Class column missing from the data.")
