...
def main():
    ...
    with tabs[6]:
        ...
        with st.expander("**🔴 Real-Time and Post-Acquisition**", expanded=False):
            st.info("If you are using MAC iOS, real-time prediction directly from the instrument is not possible, but you can drop or drag a Raw file from Waters, Bruker, or Thermo to make predictions.")
            ...
            if model_files or feature_file or label_encoder_file:
                load_all_rt(model_files, feature_file, label_encoder_file)

            # Chargement du label encoder si un fichier a été téléchargé
            if label_encoder_file is not None:
                try:
                    ...
                    # Vérifier si le label encoder est valide
                    if hasattr(label_encoder, 'classes_'):
                        class_names = label_encoder.classes_
                        st.session_state["class_names"] = class_names
                    else:
                        st.error("Le label encoder chargé ne contient pas d'attribut 'classes_'.")
                except Exception as e:
            ...
            if isinstance(class_names, (list, np.ndarray)) and len(class_names) > 0:
                st.subheader("🎨 Assign colors to labels")
                
                # Initialiser le dictionnaire de couleurs dans session_state si nécessaire
                if "label_colors" not in st.session_state:
                    st.session_state["label_colors"] = {}

                # Permettre à l'utilisateur de choisir une couleur pour chaque label
                for class_name in class_names:
                    # Utiliser un key unique pour chaque color picker
                    color_key = f"color_{class_name}"
                    
                    # Si la couleur est déjà dans session_state, la pré-remplir
                    default_color = st.session_state["label_colors"].get(class_name, "#" + ''.join([f"{i:02X}" for i in np.random.randint(0, 255, 3)]))
                    
                    # Afficher le color picker avec le key unique
                    color = st.color_picker(f"Pick a color for {class_name}", default_color, key=color_key)
                    
                    # Enregistrer la couleur sélectionnée dans session_state pour ce label
                    st.session_state["label_colors"][class_name] = color


            uploaded_file = st.file_uploader("Drag and Drop a ZIP file containing .raw or .d folders", type=["zip"])

            if uploaded_file is not None:
                temp_dir = "temp_raw_files"
                ...
                st.success(f"📦 {uploaded_file.name} successfully decompressed.")
                ...
                def detect_file_type(directory):
                    ...
                    if any(f.endswith('.raw') for f in files):
                        return 'thermo'
                    elif any(f.lower().endswith('.d') or f.lower() == 'analysis.yep' for f in files):
                        return 'bruker'
                    elif any('._FUNC' in f or 'lockspray' in f.lower() for f in files):
                        return 'waters'
                    else:
                        return None

                file_type = detect_file_type(temp_dir)
                if file_type:
                    # st.success(f"Auto-detected file type: {file_type}")
                    st.success(f"Auto-detected file type")
                else:
                    st.warning("Could not automatically detect the file type. Please verify manually.")
                    file_type = st.selectbox("Select file type manually:", ["thermo", "waters", "bruker"])

                output_dir = temp_dir

                new_files = convert_raw_to_mzml_rt_multi_format_with_zip(
                    raw_dir=temp_dir,
                ...
                mzml_files = [f for f in os.listdir(output_dir) if f.endswith(".mzML")]
                if mzml_files:
                    st.success(f"Conversion completed : {mzml_files}")
                else:
                    st.warning("No .mzML files generated after extraction and conversion.")

            raw_dir = Path("temp_raw_files")

            if st.button("Start Monitoring"):
                mzml_files = list(raw_dir.glob("*.mzML"))

                if not mzml_files:
                    st.warning("No .mzML files found in monitoring folder.")
                else:
                    for mzml_path in mzml_files:
                        st.info(f"Prediction of : {mzml_path.name}...")
                        try:
                            raw_data = load_data_single_file_rt(str(mzml_path))
                            if raw_data.empty:
                                st.error(f"No valid data in {mzml_path.name}")
                                continue

                            preprocessed = preprocess_data_rt(raw_data, normalization_type=normalization_type)
                            # binned = apply_binning_to_mass_range_rt(preprocessed)

                            result = decision_rt(preprocessed, use_svd=False)

                            st.success("Prediction completed.")
                            st.dataframe(result)
                            visualize_predictions_circles_rt(result)

                            raw_file = raw_dir / f"{mzml_path.stem}.raw"
                            mzml_path.unlink()
                            if raw_file.exists():
                                shutil.rmtree(raw_file)

                        except Exception as e:
                            st.error(f"❌ Error during {mzml_path.name} : {e}")
