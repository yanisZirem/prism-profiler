...
import gc
from session_store import SessionStore, get_store, restore_session, persist_session, start_cleanup_thread
...
st.set_page_config(
    page_title="Profiler – Multi-Omics Analysis",
    page_icon="profiler_logo.png",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': 'https://github.com/yanisZirem/prism-profiler',
        'About': 'Profiler v1.2 — Open-source omics analysis platform'
    }
)
...
.toc{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);
  padding:24px 36px;margin:32px 64px}
...
.report-section{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);
  margin:24px 64px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.04)}
...
.fig-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(min(420px,100%),1fr));gap:20px;margin:16px 0;width:100%;box-sizing:border-box}
.fig-card{overflow:hidden;width:100%;box-sizing:border-box}
...
"""
    ...
    def _fig_html(key: str, title: str) -> str:
        ...
        if kind == "plotly":
            try:
                ...
                return (
                    f'<div class="fig-card" style="overflow:hidden;position:relative;width:100%;box-sizing:border-box;">'
                    ...
                    f'</div>'
                    f'<div style="padding:4px 8px 8px;width:100%;overflow:hidden;box-sizing:border-box;">{plot_html}</div>'
                    f'</div>'
                )
    ...
    stat_figs = [
        ...
        ("boxplots_fig",  "Significant Feature Boxplots"),
    ]
    ...
    if models:
        ...
        ml_figs = [
            ...
            ("ml_comparison",      "Model Comparison"),
...
def main():
    ...
    # Start background session cleanup thread (runs once per server process)
    ...
    # Tab overflow fix (always applied)
    st.markdown("""
<style>
...
""", unsafe_allow_html=True)
    try:
        import os as _os, base64 as _b64f
        _fp = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "profiler_icons.ttf")
        if _os.path.exists(_fp):
            with open(_fp, "rb") as _ff:
                _fb = _b64f.b64encode(_ff.read()).decode()
            st.markdown(f'''<style>
@font-face {{
    font-family: "PI";
...
[data-testid="stSidebar"] .stExpander summary * {{
    ...
    try:
        ...
        with open("logo/Log2.png", "rb") as _lf:
            _lb = _b64m.b64encode(_lf.read()).decode()
        st.sidebar.markdown(f'''<div style="background:#fff;border-radius:12px;padding:10px 6px 8px;
            margin-bottom:10px;text-align:center;border:1px solid #c5d5e8;
            box-shadow:0 2px 8px rgba(49,140,231,0.1);">
            <img src="data:image/png;base64,{_lb}" style="width:100%;max-width:220px;height:auto;object-fit:contain;display:block;margin:0 auto;"/>
        </div>''', unsafe_allow_html=True)
    except FileNotFoundError:
        st.sidebar.markdown('''<div style="text-align:center;padding:12px;background:#fff;border-radius:12px;
    ...
    # ══════════════════════════════════════════════════════
    # AUTH — Login / Session / Sign Up (web-specific)
    # ══════════════════════════════════════════════════════
    ...
    class SimpleAuthenticator:
        ...
        def login(self, form_name: str):
            with st.sidebar.form(key='login_form'):
                entered_username = st.text_input(f"Email", st.session_state.get("username", ""))
                st.session_state.username = entered_username.strip().lower()
                st.session_state.password = st.text_input("Password", st.session_state.get("password", ""), type="password")
                login_button = st.form_submit_button('  Login')
            if login_button:
                user_data = self.config['credentials']['usernames'].get(st.session_state.username)
                if user_data and check_password(user_data['password'], st.session_state.password):
                    st.session_state.auth_status = True
                    st.session_state.name = user_data['name']
                    st.session_state.last_active = time.time()
                    # Create/restore persistent session on login
                    _store = get_store(st.session_state.username)
                    restore_session(_store)
                    st.rerun()
    ...
    # Session defaults
    for k, v in {"auth_status": None, "username": "", "password": "", "name": "", "last_active": time.time()}.items():
        st.session_state.setdefault(k, v)

    # ═══════════════════════════════════════════════════════════
    # SIDEBAR AUTH — Clean redesign
    # Before login: Login card + Create Account only
    # After login: greeting banner + session controls + upload guide
    # ═══════════════════════════════════════════════════════════
    ...
    if st.session_state.auth_status is not True:
        ...
        <div style='background:rgba(255,255,255,0.95);border-radius:12px;
                    padding:12px 14px 2px;border:1px solid #c5d8ee;
                    box-shadow:0 2px 10px rgba(49,140,231,0.12);margin-bottom:10px;'>
          <div style='color:#1a4a80;font-size:0.75rem;font-weight:800;text-transform:uppercase;
                      letter-spacing:0.10em;margin-bottom:8px;'>🔐 Sign In to Profiler</div>
        """, unsafe_allow_html=True)
        ...
        # Separator
        st.sidebar.markdown(
            "<div style='height:1px;background:linear-gradient(90deg,transparent,#c5d8ee,transparent);"
            "margin:6px 0 10px;'></div>", unsafe_allow_html=True
        )

        # Create Account toggle
        st.session_state.setdefault("show_signup", False)
        if st.sidebar.button("  Create an Account", key="btn_create_account", use_container_width=True):
            st.session_state.show_signup = not st.session_state.show_signup

        if st.session_state.show_signup:
            st.sidebar.markdown("""
            <div style='background:rgba(255,255,255,0.95);border-radius:12px;
                        padding:12px 14px 4px;border:1px solid #c5d8ee;
                        box-shadow:0 2px 8px rgba(49,140,231,0.1);margin-top:6px;'>
              <div style='color:#1a4a80;font-size:0.75rem;font-weight:800;text-transform:uppercase;
                          letter-spacing:0.10em;margin-bottom:8px;'>✦ New Account</div>
            """, unsafe_allow_html=True)
    ...
    # ── SESSION ACTIVE ──
    elif st.session_state.auth_status is True:
        ...
        # Restore persisted data on every rerun (survives page refresh)
        _store = get_store(st.session_state.username)
        restore_session(_store)
        _store.touch()

        st_autorefresh(interval=60000000, key="refresh")
        add_online_user(st.session_state.username)
        online_users = get_online_users()
        total_users = len(config.get('credentials', {}).get('usernames', {}))
        ...
        st.sidebar.markdown(
            f"""<div style='background:linear-gradient(135deg,rgba(49,140,231,0.14),rgba(49,140,231,0.04));
                ...
                <div style='color:#4a7ab5;font-size:0.72rem;margin-top:3px;'>
                    🟢 <b>{len(online_users)}</b> online &nbsp;·&nbsp; <b>{total_users}</b> registered
                </div>
            </div>""",
            unsafe_allow_html=True
        )
        if st.session_state.username in ["yanis.zirem@yahoo.com", "admin@example.com"]:
            with st.sidebar.expander("👥 Connected users", expanded=False):
                st.write(online_users)

        # ── Session Management — inline styled expander
        with st.sidebar.expander("  Session & Controls", expanded=True):
            ...
            with c1:
                if st.button("🔄 Reset", key="web_reset_btn", help="Clear analysis data", use_container_width=True):
                    # Clear disk cache then reinit (keep auth keys)
                    _reset_store = get_store(st.session_state.username)
                    _reset_store.cleanup()
                    keys_to_keep = {"auth_status", "username", "password", "name", "last_active"}
                    for k in [k for k in st.session_state.keys() if k not in keys_to_keep]:
                        del st.session_state[k]
                    gc.collect()
                    st.rerun()
            with c2:
                if st.button("⏹ Stop", key="global_stop_button", help="Stop current analysis", use_container_width=True):
                    st.session_state['stop_analysis'] = True
                    st.warning("⚠️ Analysis stopped.")
            with c3:
                if st.button("🚪 Exit", key="web_logout_btn", help="Logout", use_container_width=True):
                    remove_online_user(st.session_state.username)
                    # Delete persisted session data on logout
                    _logout_store = get_store(st.session_state.username)
                    _logout_store.cleanup()
                    st.session_state.clear()
                    gc.collect()
                    st.rerun()
            st.session_state.last_active = time.time()
        ...
        with st.sidebar.expander("❓ Help & Import Guide", expanded=False):
            st.markdown(
                ...
                "&nbsp;&nbsp;• &lt; 5,000 features &nbsp;·&nbsp; files &lt; 200 MB<br><br>"
                "<b>Desktop version</b><br>"
                "&nbsp;&nbsp;• ≥ 5,000 features &nbsp;·&nbsp; files &gt; 1 GB<br><br>"
                "<b>Privacy:</b> Data processed temporarily — <b>never stored</b>."
                "</div>",
                unsafe_allow_html=True
            )
            st.markdown("[⬇️ Download Desktop App](https://prism-profiler.univ-lille.fr/desktop/)")
            st.markdown("---")
            ...
            try:
                ...
                _pdf_path = next(
                    (_p for _p in [
                        _os.path.join(_os.path.dirname(__file__), "profiler_data_import_guide.pdf"),
...
&nbsp;Spectra: mzML · mzXML (MS1 tab below)
        ...
        with st.sidebar.expander("⚙️ Step 1 — RAW File Conversion", expanded=False):
            ...
            if os.path.exists(output_dir if 'output_dir' in dir() else '') and os.listdir(output_dir):
                ...
                zip_path = os.path.join("/tmp", "converted_files.zip")
                ...
                if any(group.get("files") for group in st.session_state["file_groups"]):
                    with st.spinner("🔄 Loading and processing MS files…"):
                        ...
                        if st.session_state.get("auth_status") is True:
                            _store = get_store(st.session_state.username)
                            _store.save_df("data", st.session_state["data"])
                            _store.touch()
                        del progress_bar
...
""")
        ...
        #         "<b>⚠️ Beta feature</b> — Profiler is primarily designed for cross-sectional omics data. "
        ...
        with st.sidebar.expander("📈 Step 5 — Longitudinal / Time-Series Data", expanded=False):
            st.markdown(
                "<div style='background:#fffbeb;border-left:3px solid #d97706;"
                "border-radius:6px;padding:8px 12px;font-size:0.76rem;color:#92400e;margin-bottom:8px;'>"
                "<b>Longitudinal data support</b> — Repeated-measures datasets can be loaded and analysed "
                "within the longitudinal analysis module. For advanced statistical modeling such as "
                "<b>Linear Mixed Models</b>, please navigate to:<br><br>"
                "<b>AI Modeling → Longitudinal → Linear Mixed Model</b>."
                "</div>",
                unsafe_allow_html=True
            )
            ...
            if uploaded_long is not None:
                try:
                    ...
                    if _fname.endswith((".csv", ".txt", ".tsv")):
                        ...
                        df_long = pd.read_csv(_io.BytesIO(_raw_bytes), sep=_sep, engine="python")
                    elif _fname.endswith(".xlsx"):
                        df_long = pd.read_excel(_io.BytesIO(_raw_bytes))
                    else:
                        df_long = pd.read_csv(_io.BytesIO(_raw_bytes), sep=None, engine="python")

                    if df_long is not None and not df_long.empty:
                        _subj_aliases = {"subject_id", "subjectid", "patient_id",
                                         "patientid", "subject", "patient", "individual", "animal_id"}
                        _time_aliases = {"time", "timepoint", "visit", "week", "day", "month", "year"}
                        _auto_subj = next(
                        ...
                        if _auto_time:
                            ...
                            st.success(f"✅ Time: **{_auto_time}** → {len(_tp)} timepoints: {_tp[:6]}")
                        else:
                        ...
                        with col_s:
                            _sel_subj = st.selectbox(
                                ...
                                index=0 if _auto_subj is None else _col_opts.index(_auto_subj),
                                key="long_subj_col")
                        with col_t:
                            _sel_time = st.selectbox(
                                "Time column", _col_opts,
                                index=0 if _auto_time is None else _col_opts.index(_auto_time),
                                key="long_time_col")

                        if st.button("✅ Load Longitudinal Data", key="load_long_btn",
                                     use_container_width=True):
                            _final_subj = (_sel_subj if _sel_subj != "— auto —" else _auto_subj)
                            _final_time = (_sel_time if _sel_time != "— auto —" else _auto_time)

                            if _final_subj and _final_subj != "Subject_ID":
                                df_long = df_long.rename(columns={_final_subj: "Subject_ID"})
                            if _final_time and _final_time != "Time":
                                df_long = df_long.rename(columns={_final_time: "Time"})

                            _long_protect = {"Subject_ID", "Time", "Class", "ID",
                                             "File", "RT", "Sum", "Original_index"}
                            _long_protect.update(c for c in df_long.columns if str(c).endswith("_meta"))
                            _long_feats = [c for c in df_long.columns if c not in _long_protect]
                            if _long_feats:
                            ...
                            if "Class" in df_long.columns:
                                ...
                                st.session_state["class_renaming"] = {c: c for c in _cls_list}
                            ...
                            st.success(
                                ...
                                f"{len(_tp_final)} timepoints: {sorted(_tp_final)}"
                            )
                            st.rerun()
...
<b>Key features:</b> load ROIs from imzML · bin spectra · normalize · export CSV/Excel · visualize average spectra · concatenate multiple ROI files.
            ...
            msi_zip_path = "/mnt/downloads/MSI2Profiler_Windows.zip"
    ...
    # ── Onglets conditionnels selon l'état d'authentification ──
    if st.session_state.get('auth_status') is True:
        _tabs_list = [
    ...
    with tabs[0]:
        ...
        with _lnk[0]:
            try:
                with open("documentation.pdf","rb") as _f: st.download_button("📘 Documentation",data=_f,file_name="documentation.pdf",mime="application/pdf",use_container_width=True)
            except FileNotFoundError: st.caption("📘 documentation.pdf not found")
        with _lnk[1]:
            try:
                with open("IDDN_Certificate.pdf","rb") as _f: st.download_button("⚖️ INSERM Certificate",data=_f,file_name="IDDN_Certificate.pdf",mime="application/pdf",use_container_width=True)
    ...
    if st.session_state.get('auth_status') is True:
                    ...
                    if df_overview is None or df_overview.empty:
                        ...
                        if submitted:
                            try:
                                ...
                                if st.session_state.get('auth_status') is True:
                                    _store = get_store(st.session_state.username)
                                    _store.save_df('preprocessed_data', data)
                                    _store.touch()
                ...
                with _t1_sampling:
                    ...
                    if apply_btn and technique != 'None':
                        try:
                            ...
                            if st.session_state.get('auth_status') is True:
                                _store = get_store(st.session_state.username)
                                _store.save_df('oversampled_data', st.session_state['oversampled_data'])
                                _store.touch()
            ...
            # ------------------ Undersampling ------------------
                    ...
                    if apply_btn and technique != 'None':
                        try:
                            ...
                            if st.session_state.get('auth_status') is True:
                                _store = get_store(st.session_state.username)
                                _store.save_df('undersampled_data', st.session_state['undersampled_data'])
                                _store.touch()
        ...
        with _t4_ml:
            ...
            if train_models_btn:
                ...
                try:
                    ...
                    if st.session_state.get('auth_status') is True:
                        _store = get_store(st.session_state.username)
                        _store.save_obj('models', model_results)
                        _store.touch()
        ...
        with _t4_dl:
            ...
            if X is not None:
                if st.button("Train Deep Learning Models", key="train_dl_models"):
                    try:
                        ...
                        if st.session_state.get('auth_status') is True:
                            _store = get_store(st.session_state.username)
                            _store.save_obj('dl_models', model_results)
                            _store.touch()
            ...
            #     st.plotly_chart(compare_DL(st.session_state['dl_models'], use_container_width=True, config={'displayModeBar': True, 'displaylogo': False, 'scrollZoom': True, 'modeBarButtonsToAdd': ['downloadImage'], 'toImageButtonOptions': {'format': 'png', 'scale': 2}}))
    ...
    **Requirements:**
    ...
    # ---------------- Volcano Plot Expander ----------------
                    ...
                    if not selected_features:
                        ...
                        try:
                            with st.spinner("Generating Volcano Plot..."):
                                ...
                                _capture_plotly(volcano_plot, "Features per Sample Volcano Plot")
        ...
        with tabs[6]:
            ...
            # ═══════════════════════════════════════════════════
            from profiler_genes_enrichement import render_enrichment_tab
